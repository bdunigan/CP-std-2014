ng.ready(function(){
	...
	//--------- form validation
	ng.get('buyBtn').add_event('click', function(evt){
		var errs = [];
	...
	// --------------BRYAN added validation 		
	 if (ng.get('matchproof').checked) {
					...
					if (ng.get('pfapp_photo_zoom').value == ''){
						...
						if ((val < 0) || (val > 100)) {
							errs.push(' - Photo zoom must be between 0% to 100%');		
						}
					}
			 };
...
//--bryan new submit event
//ng.get('pfapp_form').add_event('submit', function(evt){
		//alert("event form submission");
		//	  return true;
		//});
	...
	function set_frame_size(size, label){
		...
		// frame size title
		ng.get('pfapp_frame_title').set_html(label+' Frame');
